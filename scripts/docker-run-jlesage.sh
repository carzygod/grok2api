...
docker run -d --name "grok2api-browser-${PROFILE}" \
  ...
  -e TZ="${GROK2API_BROWSER_TIMEZONE:-Asia/Taipei}" \
  ${GROK2API_BROWSER_PROXY_SERVER:+-e CHROME_PROXY_SERVER="${GROK2API_BROWSER_PROXY_SERVER}"} \
  ${GROK2API_BROWSER_PROXY_BYPASS_LIST:+-e CHROME_PROXY_BYPASS_LIST="${GROK2API_BROWSER_PROXY_BYPASS_LIST}"} \
